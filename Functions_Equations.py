...
def Moment_y(x):
    return -R1z*positive(x-x1)+P*np.cos(theta)*positive(x-x2-xa/2)-R2z*positive(x-x2)-R3z*positive(x-x3)-Ractuator*np.cos(theta)*positive(x-x2+xa/2)

def Moment_z(x): #+q
    return -R1y*positive(x-x1)-R2y*positive(x-x2)-R3y*(x-x3)+P*np.sin(theta)*positive(x-x2-xa/2)-Ractuator*np.sin(theta)*positive(x-x2+xa/2)
...
def twist(x):
    return 1/(G*J)*(-R1y*positive(x-x1)*(-ha/2-z_centroid)-R2y*positive(x-x2)*(-ha/2-z_centroid)-R3y*positive(x-x3)*(-ha/2-z_centroid)+P*np.sin(theta)*positive(x-x2-xa/2)*(-z_centroid)-P*np.cos(theta)*positive(x-x2-xa/2)*ha/2-Ractuator*np.sin(theta)*positive(x-x2+xa/2)*(-z_centroid))+C5